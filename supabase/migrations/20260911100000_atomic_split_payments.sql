-- One request records an enrollment/renewal/collection and all its payment rows.
...
create policy "owner payment operations read" on public.payment_operations for select to authenticated
...
alter table public.payments add column operation_id uuid,
  add constraint payments_operation_gym_fk foreign key(gym_id, operation_id) references public.payment_operations(gym_id, id);
create index payments_operation_idx on public.payments(gym_id, operation_id) where operation_id is not null;

create function public.submit_payment_operation(p_request_id uuid, p_kind text, p_details jsonb, p_payments jsonb)
...
begin
  select * into g from public.gyms where id = public.current_gym_id() and owner_id = auth.uid() and is_active for update;
  ...
  if p_kind = 'collect' then
    ...
    if p_kind='activate' then
      ...
      update public.members set whatsapp_reminders_enabled=coalesce((p_details->>'whatsapp_reminders_enabled')::boolean,false) where id=member_id and gym_id=g.id;
    else
  ...
  -- The operation and all financial rows roll back together on any error.
  ...
  for row in select value from jsonb_array_elements(p_payments) loop
    ...
    update public.payments set operation_id=op.operation_id where id=payment.id and gym_id=g.id;
