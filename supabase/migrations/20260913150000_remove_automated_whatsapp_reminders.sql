-- Remove the automated WhatsApp reminder scheduler and delivery ledger.
-- Manual owner-triggered WhatsApp reminder handoffs remain supported through manual_reminder_events.
...
drop function if exists public.invoke_payment_reminders_cron();
...
alter table if exists public.gyms
  drop column if exists automatic_payment_whatsapp_enabled,
  drop column if exists whatsapp_payment_template_name,
  drop column if exists whatsapp_template_language,
  drop column if exists automatic_payment_emails_enabled,
  drop column if exists payment_reminder_subject,
