-- Revalidate membership eligibility when claiming; reverted renewals are not operational.
  ...
  with valid_target as (
    ...
    where ms.id = p_membership_id
      ...
      and not exists (
        ...
        where future.gym_id = ms.gym_id and future.member_id = ms.member_id
          and future.reverted_at is null and future.starts_on > p_scheduled_for
      )
      and exists (
        select 1 from public.members member
        where member.id = ms.member_id and member.gym_id = ms.gym_id
