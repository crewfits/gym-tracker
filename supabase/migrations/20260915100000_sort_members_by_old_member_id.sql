-- The Member ID column is searchable and needs a database-backed sort so
-- sorting remains correct across pages in the directory and CSV export.
...
as $$
  ...
  order by
    ...
    case when p_sort = 'old_member_id' and lower(p_order) = 'asc' then lower(filtered.old_member_id) end asc nulls last,
    case when p_sort = 'old_member_id' and lower(p_order) = 'desc' then lower(filtered.old_member_id) end desc nulls last,
    ...
    case when coalesce(p_sort, 'created_at') not in ('member_code', 'old_member_id', 'name', 'expires_on', 'balance', 'status') and lower(coalesce(p_order, 'desc')) = 'asc' then filtered.created_at end asc,
    case when coalesce(p_sort, 'created_at') not in ('member_code', 'old_member_id', 'name', 'expires_on', 'balance', 'status') and lower(coalesce(p_order, 'desc')) <> 'asc' then filtered.created_at end desc,
