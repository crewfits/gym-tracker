-- Optional private member profile photos for QR/operator verification.
...
create policy "owner member photo read" on storage.objects
  for select
  using (
...
create policy "owner member photo insert" on storage.objects
  for insert
  with check (
    bucket_id = 'member-photos'
    and split_part(name, '/', 1) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
...
create policy "owner member photo update" on storage.objects
  for update
  using (
    bucket_id = 'member-photos'
    and split_part(name, '/', 1) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
...
create policy "owner member photo delete" on storage.objects
  for delete
  using (
    bucket_id = 'member-photos'
    and split_part(name, '/', 1) ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
