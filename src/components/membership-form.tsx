...
export function MembershipForm({ memberId, plans, trainers = [], showTrainerAssignment = false, handlers = [], defaultHandlerId = "", action, today, renew = false, embedded = false, currentExpiry, defaultPlanId, defaultTrainerId, error, returnPath }: { memberId: string; plans: Plan[]; trainers?: TrainerOption[]; showTrainerAssignment?: boolean; handlers?: StaffHandlerOption[]; defaultHandlerId?: string; action: (data: FormData) => Promise<SplitPaymentResult>; today: string; renew?: boolean; embedded?: boolean; currentExpiry?: string; defaultPlanId?: string | null; defaultTrainerId?: string | null; error?: string; returnPath?: string }) {
  ...
  return <form onSubmit={submit} aria-busy={pending} className={embedded ? "form member-renewal-form" : "card form"} style={embedded ? undefined : { maxWidth: 800 }} onInvalid={(event) => {
    ...
    <PaymentEntries rows={rows} onChange={changeRows} balancePaise={totalPaise} today={today} disabled={pending}/>
    <div className="membership-form-footer">
      <div><span className="muted">Total</span><strong>{formatInr(totalPaise)}</strong></div>
