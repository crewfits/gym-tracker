...
export function PaymentEntries({ rows, onChange, balancePaise, today, required = false, disabled = false }: {
  rows: PaymentRow[]; onChange: (rows: PaymentRow[]) => void; balancePaise: number; today: string; required?: boolean; disabled?: boolean;
}) {
  const id = useId();
  ...
  return <div className="payment-entries">
    ...
    {rows.map((row, index) => <fieldset key={row.key} disabled={disabled} className="payment-entry">
      ...
      <div className="form-grid">
        <div className="field"><label htmlFor={`${id}-${index}-amount`}>Amount (₹) <span className="required-marker">*</span></label><input {...attrs(index, "amount")} type="number" inputMode="decimal" min="0.01" step="0.01" value={row.amount} onChange={event => update(index, "amount", event.target.value)} required/>{feedback(index, "amount")}</div>
    ...
    <div className="payment-entry-totals" aria-live="polite"><span>Total paid <strong>{formatInr(total)}</strong></span><span>Remaining balance <strong>{formatInr(Math.max(0, balancePaise - total))}</strong></span></div>
