...
export async function GET(request: NextRequest) {
  ...
  const csv = csvDocument(
    ["Business date", "Occurred at", "Member ID", "Member name", "Movement", "Membership", "QR version"],
    rows.map((row) => [row.business_date, new Intl.DateTimeFormat("en-IN", { timeZone: gym.timezone, dateStyle: "medium", timeStyle: "long" }).format(new Date(row.occurred_at)), row.member_code, row.member_name, row.direction, row.plan_name, row.qr_version]),
  );
