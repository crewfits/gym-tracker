...
export default async function PublicReceiptPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  })}<tr><td><strong>Paid on this receipt</strong></td><td><strong>{formatInr(totalReceivedPaise, currencyCode)}</strong></td></tr>{balanceDuePaise > 0 && <tr><td><strong>Balance due</strong></td><td><strong>{formatInr(balanceDuePaise, currencyCode)}</strong></td></tr>}</tbody></table><small className="muted">This private receipt link was shared by {gym.name}. Do not forward it unless needed.</small></div></main>;
}
