...
export default async function PublicReceiptPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: payment } = await db.from("payments").select("id,charge_id,amount_paise,method,reference,paid_on,receipt_number,voided_at,operation_id,payment_reversals(amount_paise),gyms(name,address,phone,email,gstin),charges!inner(subtotal_paise,discount_paise,gst_rate_basis_points,tax_paise,total_paise,memberships!inner(plan_name,starts_on,expires_on,members!inner(name,member_code)))").eq("id", paymentId).maybeSingle();
  if (!payment) notFound();
  ...
  })}<tr><td><strong>Paid on this receipt</strong></td><td><strong>{formatInr(totalReceivedPaise)}</strong></td></tr><tr><td><strong>Balance due</strong></td><td><strong>{formatInr(balanceDuePaise)}</strong></td></tr></tbody></table><small className="muted">This private receipt link was shared by {gym.name}. Do not forward it unless needed.</small></div></main>;
}
