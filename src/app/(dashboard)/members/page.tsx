...
import { ArrowDownToLine, Plus, Share2, RefreshCw, UserRoundPen } from "lucide-react";
...
export default async function Members({ searchParams }: PageProps<"/members">) {
  ...
  return <>
    ...
    <div className="card table-wrap members-table-card"><table className="table"><colgroup><col className="member-column-name"/><col className="member-column-contact"/><col className="member-column-plan"/><col className="member-column-expiry"/><col className="member-column-balance"/><col className="member-column-status"/><col className="member-column-qr"/><col className="member-column-actions"/></colgroup><thead><tr><SortableTableHeader label="Member" href={hrefForSort("name", "asc")} active={sort === "name"} order={order}/><th>Contact</th><th>Plan</th><SortableTableHeader label="Plan end" href={hrefForSort("expires_on", "asc")} active={sort === "expires_on"} order={order}/><SortableTableHeader label="Balance" href={hrefForSort("balance", "desc")} active={sort === "balance"} order={order}/><SortableTableHeader label="Status" href={hrefForSort("status", "asc")} active={sort === "status"} order={order}/><th>QR</th><th>Actions</th></tr></thead><tbody>{rows.map((member) => {
      ...
      return <tr key={member.id}><td><Link className="member-cell" href={`/members/${member.id}`}><span className="member-avatar small">{photoUrl ? <Image src={photoUrl} alt="" width={38} height={38} unoptimized/> : member.name.slice(0, 1).toUpperCase()}</span><span className="member-name-copy"><strong title={member.name}>{member.name}</strong><small className="muted">{member.member_code}</small></span></Link></td><td><span className="member-contact-phone">{member.phone}</span>{member.email && <small className="muted member-contact-email" title={member.email}>{member.email}</small>}</td><td><span className="member-plan-name" title={effectiveMembership?.plan_name ?? member.plan_name ?? undefined}>{effectiveMembership?.plan_name ?? member.plan_name ?? "—"}</span></td><td>{formatDisplayDate(effectiveMembership?.expires_on ?? member.expires_on)}</td><td>{formatInr(Number(member.balance_paise))}</td><td><span className={`badge ${displayStatus}`}>{displayStatus.replaceAll("_", " ")}</span></td><td><span className={`badge ${qrBadgeClass}`}>{qrStatus}</span></td><td><div className="inline-actions member-row-actions"><Link className="button secondary small member-action-icon" href={`/members/${member.id}`} title="Manage member" aria-label={`Manage ${member.name}`}><UserRoundPen size={16}/></Link>{!member.is_archived && <Link className="button secondary small member-action-icon" href={`/members/${member.id}/qr`} title={member.qr_enabled ? "Share QR pass & receipts" : "Generate QR pass"} aria-label={`${member.qr_enabled ? "Share QR pass and receipts for" : "Generate QR pass for"} ${member.name}`}><Share2 size={16}/></Link>}{!member.is_archived && member.qr_enabled && <ConfirmActionForm action={issueMemberQr} memberId={member.id} message={`Regenerate ${member.name}'s QR? Every old copy will stop working.`} className="button danger small member-action-icon" label={`Regenerate QR for ${member.name}`} icon={<RefreshCw size={16}/>} pendingLabel="Regenerating…"/>}</div></td></tr>;
