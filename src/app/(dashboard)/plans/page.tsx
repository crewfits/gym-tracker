...
export default async function Plans({ searchParams }: PageProps<"/plans">) {
  ...
  const activeCount = sortedPlans.filter((plan) => plan.is_active).length;
  ...
  return <>
    <div className="page-head">
      <div>
        ...
        <p className="muted">{activeCount} active plans · changes apply only to future enrollments and renewals.</p>
      </div>
    </div>
    <Feedback success={success} error={error}/>
    <div className="plans-layout">
      <section className="card plans-table-card">
        <div className="plans-table-head">
          <div><h2>Plan catalogue</h2><p className="muted">Edit prices and durations without changing old receipts.</p></div>
          <span>{sortedPlans.length} plans</span>
        </div>
        <div className="table-wrap">
          <table className="table">
            <thead><tr><SortableTableHeader label="Plan" href={hrefForSort("name", "asc")} active={sort === "name"} order={order}/><SortableTableHeader label="Duration" href={hrefForSort("duration", "asc")} active={sort === "duration"} order={order}/><SortableTableHeader label="Fee" href={hrefForSort("fee", "desc")} active={sort === "fee"} order={order}/><SortableTableHeader label="Status" href={hrefForSort("status", "desc")} active={sort === "status"} order={order}/><th>Actions</th></tr></thead>
            <tbody>{sortedPlans.map((plan) => <tr key={plan.id}>
              ...
              <td><div className="inline-actions">
                ...
                <form action={togglePlan}>
                  ...
                  <input type="hidden" name="active" value={String(!plan.is_active)}/>
                  <SubmitButton className="button secondary small" pendingLabel={plan.is_active ? "Archiving..." : "Activating..."}>{plan.is_active ? "Archive" : "Activate"}</SubmitButton>
                </form>
              </div></td>
            </tr>)}</tbody>
          </table>
        </div>
        {!plans?.length && <div className="empty">Create your first membership plan.</div>}
      </section>

      <form action={createPlan} className="card form plans-create-card">
        <div>
          <h2>Add plan</h2>
          <p className="muted">Create a reusable plan for new members and renewals.</p>
        </div>
        <div className="field"><label>Name</label><input name="name" required placeholder="Monthly"/></div>
