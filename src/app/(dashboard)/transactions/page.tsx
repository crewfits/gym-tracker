...
import { ArrowDownToLine, Banknote, CircleDollarSign, RefreshCw, RotateCcw } from "lucide-react";
...
export default async function Transactions({ searchParams }: PageProps<"/transactions">) {
  ...
  const { data, error } = await supabase.rpc("list_transactions", { p_query: q || null, p_method: methodValue, p_status: status || null, p_from: from, p_to: to, p_page: page, p_page_size: pageSize, p_sort: sort, p_order: order });
  ...
  return <>
    ...
    <div className="pagination"><span className="muted">{total ? `${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, total)} of ${total}` : "0 transactions"}</span><div>{page > 1 && <Link className="button secondary small" href={`/transactions?${queryFor({ targetPage: page - 1 }).toString()}`}>Previous</Link>}{page < pages && <Link className="button secondary small" href={`/transactions?${queryFor({ targetPage: page + 1 }).toString()}`}>Next</Link>}</div></div>
  </>;
}
