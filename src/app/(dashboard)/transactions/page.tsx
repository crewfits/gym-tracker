...
import { businessDate, formatCompactMoney, formatDisplayDate, formatInr, formatPaymentMethod, normalizeCurrencyCode } from "@/lib/domain";
    ...
    <section className="transaction-ledger-row">
      <div className="card ledger-command-card">
        ...
        <div className="ledger-mini-grid">
          ...
          <div><RotateCcw size={18}/><span>Reversed</span><strong title={formatInr(Number(totals?.view_reversed_paise ?? 0), currencyCode)}>{formatCompactMoney(Number(totals?.view_reversed_paise ?? 0), currencyCode)}</strong></div>
          <div><CircleDollarSign size={18}/><span>Outstanding</span><strong title={formatInr(Number(summary?.outstanding_paise ?? 0), currencyCode)}>{formatCompactMoney(Number(summary?.outstanding_paise ?? 0), currencyCode)}</strong></div>
