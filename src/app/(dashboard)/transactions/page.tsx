  ...
  return <>
    ...
    <section className="transaction-ledger-row">
      <div className="card ledger-command-card">
        <div className="ledger-command-head"><span className="metric-icon"><CircleDollarSign size={21}/></span><div><span className="metric-label">Collected in this view</span><div className="metric">{formatInr(Number(totals?.view_collected_paise ?? 0))}</div></div></div>
      ...
      <div className="card ledger-mix-card">
        ...
        <div className="ledger-method-grid">{methodAmounts.map(([method, amount]) => {
          ...
          return <div className={`ledger-method-tile ${method}`} key={method}><div className="method-tile-head"><span className="method-icon"><Icon size={17}/></span><span>{formatPaymentMethod(method)}</span><strong>{methodTotal ? `${Math.round((amount / methodTotal) * 100)}%` : "0%"}</strong></div><div><strong>{formatInr(amount)}</strong><small>{amount === methodMax && amount > 0 ? "Top channel" : "Collection channel"}</small></div></div>;
        })}</div>
      </div>
    </section>
