...
export default async function RemindersPage({ searchParams }: { searchParams: Promise<{ filter?: string; page?: string; error?: string }> }) {
  ...
  return <>
    ...
    <form className="toolbar"><select className="search" name="filter" defaultValue={selectedFilter}><option value="">All action queues</option><option value="overdue">Overdue payments</option><option value="partial_payment">Partial payment reminders</option><option value="expiring">Expiring in 7 days</option></select><button className="button secondary">Filter</button></form>
...
function reminderLabel(kind: string) {
  return ({ overdue: "Overdue payment", partial_payment: "Partial payment reminder", expiring: "Expiring in 7 days" } as Record<string, string>)[kind] ?? kind.replaceAll("_", " ");
}
