...
export default async function AttendancePage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; direction?: string; view?: string; from?: string; to?: string }> }) {
  ...
  return <>
    <div className="page-head"><div><p className="eyebrow">Access ledger</p><h1>Attendance</h1><p className="muted">{today} · Today&apos;s check-ins, check-outs and searchable history.</p></div><a className="button secondary" href={`/api/exports/attendance?${queryFor().toString()}`}><ArrowDownToLine size={16}/> Export this view</a></div>
