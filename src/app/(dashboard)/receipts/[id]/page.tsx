...
export default async function ReceiptPage({ params, searchParams }: PageProps<"/receipts/[id]">) {
  ...
  return <div className="receipt">
    ...
    <div className="receipt-actions no-print"><PrintButton/><WhatsAppReceiptButton url={whatsappUrl}/><ShareReceiptButton receiptNumber={payment.receipt_number} url={publicUrl}/>{member.email && <form action={emailReceipt}><input type="hidden" name="payment_id" value={payment.id}/><button className="button secondary">Email to {member.email}</button></form>}</div>
  </div>;
}
