...
- `paid_amount` is optional. When it is greater than zero, `payment_method` and `payment_date` are required. FitKiro records one explicitly labelled controlled-import payment; it does not fabricate historical installments.
